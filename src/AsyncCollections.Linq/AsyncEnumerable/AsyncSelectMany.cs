...
public static partial class AsyncEnumerable {
    ...
    private static async IAsyncEnumerable<E> AsyncSelectManyHelper<T, E>(
        this IAsyncEnumerable<T> sequence, 
        Func<T, ValueTask<IAsyncEnumerable<E>>> selector,
        [EnumeratorCancellation] CancellationToken cancellationToken = default) {

        await foreach (var item in sequence.WithCancellation(cancellationToken)) {
            var subSequence = await selector(item);

            await foreach (var sub in subSequence.WithCancellation(cancellationToken)) {
                yield return sub;
            }
        }
    }
